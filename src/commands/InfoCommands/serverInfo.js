...
const SERVER_ID = "5j433z"; // ID server FiveM kamu
...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (sub === "fivem") {
            ...
            const pageSize = 10; // jumlah player per halaman
            let currentPage = 0;

            // helper untuk bikin embed halaman tertentu
            const generateEmbed = (page) => {
                ...
                return new EmbedBuilder()
                    ...
                    .addFields(
                        { name: "IP / Connect", value: data.connectEndPoints?.[0] || "N/A", inline: true },
                        ...
                        { name: "Performance", value: `Ping: ${data.ping || "N/A"}ms\nLast Seen: <t:${Math.floor((data.lastSeen || 0) / 1000)}:R>` },
            ...
            const getRow = () => new ActionRowBuilder().addComponents(
                ...
                new ButtonBuilder()
                    ...
                    .setLabel("Next ▶️")
                    .setStyle(ButtonStyle.Secondary)
                    .setDisabled((currentPage + 1) * pageSize >= players.length)
            );
            ...
            collector.on("collect", async (i) => {
                if (i.user.id !== interaction.user.id) {
                    return i.reply({ content: "❌ Kamu tidak bisa pakai tombol ini.", ephemeral: true });
                }
